...
class OSCServer(Thread):
    def __init__(self, ipAddr='127.0.0.1', port=9001):
        ...
        try:
            ...
            print(
                'there was an error binding to ip %s and port %d, maybe the port is already taken by another process?' % (
                ipAddr, port))
...
def createListener(ipAddr='127.0.0.1', port=9001):
    ...
    try:
        ...
        print(
            'there was an error binding to ip %s and port %i , maybe the port is already taken by another process?' % (
            ipAddr, port))
        return 0
